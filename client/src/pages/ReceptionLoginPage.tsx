...
import { AlertCircle, Users, Search, KeyRound, Clock, Phone } from "lucide-react";
    ...
    const handleUsernameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (username.length >= 3) {
            ...
            setSelectedStaff(staff || {
                ...
                position: "Front Desk Professional",
                experience: "Service Excellence",
                image: "https://images.unsplash.com/photo-1556740734-75474a702e19?w=400&h=400&fit=crop&crop=face",
            });
            setShowProfile(true);
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await login(formData.username, formData.password, {
                ...
                receptionistName: selectedStaff?.name || "Receptionist"
            });
                ...
                <div className="w-full bg-white rounded-[1.2rem] md:rounded-[2.5rem] shadow-[0_10px_20px_-8px_rgba(0,0,0,0.06)] md:shadow-[0_25px_50px_-10px_rgba(0,0,0,0.06)] border border-slate-100 flex flex-col overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-blue-700 to-indigo-800 p-4 pb-8 md:p-6 md:pt-8 md:pb-12 relative">
                        ...
                        <div className="flex flex-col items-center justify-center gap-0.5 md:gap-1.5 text-white">
                            <div className="flex items-center gap-1.5 md:gap-2">
                                <Users className="h-3.5 w-3.5 md:h-5 md:w-5 opacity-80" />
                                <h1 className="text-lg md:text-2xl font-black uppercase tracking-tight">Receptionist Portal</h1>
                            </div>
                            <p className="text-blue-100 font-bold text-[8px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em]">Front Desk Authentication</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row p-4 pt-6 md:p-8 lg:p-10 gap-6 lg:gap-10 mt-[-20px] md:mt-[-30px] bg-white rounded-t-[1.2rem] md:rounded-[2.5rem]">
                        ...
                        <div className="flex-1 space-y-6 md:space-y-8">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-1 md:space-y-1.5">
                                    <Label className="ml-4 md:ml-5 text-[8px] md:text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Staff ID *</Label>
                                ...
                                >
                                    {loginLoading ? <div className="animate-spin rounded-full h-4 w-4 md:h-5 md:w-5 border-b-2 border-white" /> : <KeyRound className="h-4 w-4 md:h-5 md:w-5" />}
                                    RECEPTION LOGIN
                                </Button>
                            </form>
                            ...
                            <div className="p-3 md:p-4 bg-blue-50/50 rounded-[1rem] md:rounded-[2rem] border border-blue-100 space-y-1.5 md:space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-[8px] md:text-[9px] font-black text-blue-400 uppercase tracking-widest">Demo Account</p>
                                </div>
                                <div className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-blue-600/80">
                                    REC001: <span className="text-blue-400">RECEPTION2024</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col items-center">
                            <div className="w-full h-full min-h-[250px] md:min-h-[400px] bg-gradient-to-br from-slate-50 to-blue-50/30 rounded-[1.2rem] md:rounded-[2.5rem] border-2 border-dashed border-slate-200 p-4 md:p-8 flex flex-col items-center justify-center text-center shadow-inner relative overflow-hidden group">
                                {showProfile && selectedStaff ? (
                                    <div className="space-y-6 md:space-y-10 animate-in fade-in zoom-in duration-700">
                                        <div className="relative">
                                            <div className="absolute inset-0 bg-slate-600 rounded-full blur-[30px] opacity-10 group-hover:opacity-20 transition-opacity" />
                                    ...
                                    <div className="space-y-3 md:space-y-4 opacity-30">
                                        <div className="w-24 h-24 md:w-48 md:h-48 rounded-full bg-slate-100 flex items-center justify-center border-[2px] md:border-[4px] border-white shadow-lg mx-auto">
                                            <Users className="h-10 w-10 md:h-24 md:w-24 text-slate-200" />
                                        </div>
                                        <div className="space-y-0.5 md:space-y-1">
                                            <p className="text-base md:text-xl font-black text-slate-300 uppercase tracking-tighter">Staff Details</p>
                                            <p className="text-[8px] md:text-[9px] font-bold text-slate-300 uppercase tracking-widest">Enter ID to begin</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="absolute bottom-3 md:bottom-6 left-3 right-3 md:left-6 md:right-6 bg-white/80 backdrop-blur p-3 md:p-4 rounded-[0.8rem] md:rounded-[1.5rem] border border-blue-50 shadow-sm flex gap-2 md:gap-3 items-start text-left">
                                    <Clock className="h-3.5 w-3.5 md:h-4 md:w-4 text-indigo-400 shrink-0 mt-0.5" />
                                    <div>
                                        ...
                                        <p className="text-[7px] md:text-[8px] font-bold text-slate-400 leading-tight uppercase">
                                            All front desk activities are recorded.
                                        </p>
                                    </div>
                                </div>
                            </div>
