...
import { AlertCircle, Heart, Search, KeyRound, Clock, Siren } from "lucide-react";
    ...
    const handleUsernameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (username.length >= 3) {
            ...
            setSelectedStaff(staff || {
                ...
                position: "Nursing Professional",
                experience: "Patient Care Specialist",
                image: "https://images.unsplash.com/photo-1584432851737-f8313426210f?w=400&h=400&fit=crop&crop=face",
            });
            setShowProfile(true);
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await login(formData.username, formData.password, {
                ...
                nurseName: selectedStaff?.name || "Nurse"
            });
                    ...
                    <div className="bg-gradient-to-r from-blue-700 to-indigo-800 p-4 pb-8 md:p-6 md:pt-8 md:pb-12 relative">
                        ...
                        <div className="flex flex-col items-center justify-center gap-0.5 md:gap-1.5 text-white">
                            <div className="flex items-center gap-1.5 md:gap-2">
                                <Heart className="h-3.5 w-3.5 md:h-5 md:w-5 opacity-80" />
                                <h1 className="text-lg md:text-2xl font-black uppercase tracking-tight">Nursing Portal</h1>
                            </div>
                            <p className="text-blue-100 font-bold text-[8px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em]">Patient Care Authentication</p>
                        </div>
                    </div>
                        ...
                        <div className="flex-1 space-y-6 md:space-y-8">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-1 md:space-y-1.5">
                                    <Label className="ml-4 md:ml-5 text-[8px] md:text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Nurse ID *</Label>
                                ...
                                <div className="space-y-1 md:space-y-1.5">
                                    <Label className="ml-4 md:ml-5 text-[8px] md:text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Security Pin *</Label>
                                    <Input
                                        type="password"
                                        name="password"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                                    <div className="space-y-1.5">
                                        <Label className="ml-5 text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">Duty Shift *</Label>
                                        <select
                                            name="shift"
                                ...
                                >
                                    {loginLoading ? <div className="animate-spin rounded-full h-4 w-4 md:h-5 md:w-5 border-b-2 border-white" /> : <Siren className="h-4 w-4 md:h-5 md:w-5" />}
                                    NURSE LOGIN
                                </Button>
                            </form>
                            ...
                            <div className="p-3 md:p-4 bg-blue-50/50 rounded-[1rem] md:rounded-[2rem] border border-blue-100 space-y-1.5 md:space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-[8px] md:text-[9px] font-black text-blue-400 uppercase tracking-widest">Demo Staff</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-[8px] md:text-[9px] font-black uppercase tracking-widest">
                                    <p className="text-blue-600">NUR001: <span className="text-blue-400">NURSE2024</span></p>
                                    <p className="text-blue-600">NUR002: <span className="text-blue-400">PAT2024</span></p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="w-full h-full min-h-[250px] md:min-h-[400px] bg-gradient-to-br from-slate-50 to-indigo-50/30 rounded-[1.2rem] md:rounded-[2.5rem] border-2 border-dashed border-slate-100 p-4 md:p-8 flex flex-col items-center justify-center text-center shadow-inner relative overflow-hidden group">
                                {showProfile && selectedStaff ? (
                                    ...
                                    <div className="space-y-3 md:space-y-4 opacity-30">
                                        <div className="w-24 h-24 md:w-48 md:h-48 rounded-full bg-teal-100 flex items-center justify-center border-[2px] md:border-[4px] border-white shadow-lg mx-auto">
                                            <Heart className="h-10 w-10 md:h-24 md:w-24 text-teal-200" />
                                        </div>
                                        <div className="space-y-0.5 md:space-y-1">
                                            <p className="text-base md:text-xl font-black text-teal-300 uppercase tracking-tighter">Nurse Identity</p>
                                            <p className="text-[8px] md:text-[9px] font-bold text-teal-300 uppercase tracking-widest">Enter ID to begin</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="absolute bottom-3 md:bottom-6 left-3 right-3 md:left-6 md:right-6 bg-white/80 backdrop-blur p-3 md:p-4 rounded-[0.8rem] md:rounded-[1.5rem] border border-blue-50 shadow-sm flex gap-2 md:gap-3 items-start text-left">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[7px] md:text-[8px] font-bold text-slate-400 leading-tight uppercase">
                                            Clinical access is monitored. Shift accuracy is required.
                                        </p>
                                    </div>
                                </div>
                            </div>
