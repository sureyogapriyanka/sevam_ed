...
export default function PharmacistDashboard() {
    ...
    return (
        <div className="bg-slate-950 min-h-screen text-slate-200 p-4 md:p-8 space-y-6">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-600/20 via-slate-900/40 to-blue-600/20 border border-white/10 p-8 shadow-2xl backdrop-blur-3xl group">
                <div className="absolute top-0 right-0 w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 group-hover:bg-blue-500/20 transition-all duration-1000"></div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-blue-400 font-black text-[10px] uppercase tracking-[0.3em]">Operational Status: Active</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4 pt-2">
                            <Badge className="bg-white/5 text-blue-300 border border-white/10 py-1.5 px-4 backdrop-blur-md rounded-xl font-black text-[10px] tracking-widest uppercase">
                                Staff: {user?.name || 'Authorized Personnel'}
                            </Badge>
                            <div className="flex items-center gap-2 text-slate-500 font-bold text-[10px] uppercase tracking-widest">
                                <Clock className="h-3.5 w-3.5 text-blue-500" /> {format(currentTime, 'hh:mm:ss a')}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            <RefreshCw className="h-4 w-4 mr-2 text-blue-400" /> DATA SYNC
                        </Button>
                        <Button
                            variant="destructive"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {quickStats.map((stat, i) => (
                    <Card key={i} className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 shadow-2xl hover:bg-slate-900/60 transition-all duration-500 group rounded-[2rem] overflow-hidden relative">
                        <div className={`absolute top-0 right-0 w-24 h-24 ${stat.color.replace('text', 'bg').replace('-600', '-500')}/10 rounded-full blur-3xl group-hover:blur-2xl transition-all`}></div>
                        <CardContent className="p-6 flex items-center justify-between relative z-10">
                            ...
                            <div className={`${stat.color.replace('text', 'bg').replace('-600', '-500')}/20 ${stat.color} p-4 rounded-2xl group-hover:scale-110 group-hover:rotate-6 transition-all border border-white/5`}>
                                <stat.icon size={24} />
                            </div>
                        </CardContent>
                    </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <div className="space-y-1">
                            ...
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-12">Authorized medication fulfillment</p>
                        </div>
                        <Badge className="bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 font-black px-6 py-2 rounded-xl text-xs backdrop-blur-md">
                    ...
                    <div className="relative group">
                        ...
                        <Input
                            ...
                            className="h-14 pl-12 rounded-2xl border-white/5 bg-slate-900/40 backdrop-blur-3xl text-white font-bold text-sm focus:ring-2 focus:ring-blue-500/50 transition-all shadow-2xl"
                            value={searchTerm}
                    ...
                    {loadingRx ? (
                        ...
                        <div className="py-20 text-center bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-white/5 shadow-2xl">
                            <div className="relative inline-block">
                                <CheckCircle2 className="h-24 w-24 text-emerald-500/20 animate-ping absolute inset-0" />
                                <CheckCircle2 className="h-24 w-24 text-emerald-500/40 relative" />
                            </div>
                            <p className="text-white font-black text-3xl tracking-tighter mt-6">FULFILLMENT COMPLETE</p>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {filteredRx.map((apt: any) => {
                                ...
                                return (
                                    <div key={apt._id} className="group relative bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] hover:bg-slate-900/60 transition-all duration-500 hover:border-blue-500/30 overflow-hidden cursor-pointer shadow-xl">
                                        <div className={`absolute top-0 left-0 w-2 h-full ${isDispensed ? 'bg-slate-700' : 'bg-blue-600'} opacity-50 shadow-[0_0_20px_rgba(37,99,235,0.3)]`} />

                                        <div className="flex flex-col lg:flex-row items-center p-6 gap-8 text-white">
                                            <div className="flex flex-col items-center justify-center min-w-[80px] h-[80px] bg-white/5 rounded-3xl border border-white/5 group-hover:border-white/10 transition-colors">
                                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">TOKEN</span>
                                                <span className="text-3xl font-black italic">{apt.tokenNumber || '—'}</span>
                                            </div>
                                            ...
                                            <div className="flex-1 space-y-2">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <Badge className={`rounded-xl px-3 py-1 text-[10px] font-black uppercase tracking-widest border-none ${isDispensed ? 'bg-slate-800 text-slate-500' : 'bg-blue-500/20 text-blue-400'}`}>
                                                        {apt.status?.toUpperCase()}
                                                    </Badge>
                                                </div>
                                                <div className="flex flex-wrap gap-4 items-center">
                                                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                                        Authority: <span className="text-slate-200">Dr. {doctorName}</span>
                                                    </p>
                                                    ...
                                                    {apt.completedAt && (
                                                        <>
                                                            ...
                                                            <div className="flex items-center gap-1.5 text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                                                                <Clock className="h-3 w-3 text-blue-500" /> {formatDistanceToNow(new Date(apt.completedAt))} ago
                                                            </div>
                                                        </>
                                                    )}
                                                </div>
                                            ...
                                            <div className="flex flex-row lg:flex-col items-center lg:items-end gap-3 w-full lg:w-auto">
                                                ...
                                                <div className="flex gap-2 w-full lg:w-auto">
                                                    <Button
                                                        variant="outline"
                                                        className="flex-1 h-12 bg-white/5 border-white/10 text-slate-300 hover:bg-white/10 hover:text-white rounded-2xl px-6 font-black transition-all"
                ...
                <div className="space-y-6">
                    <Card className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 shadow-2xl rounded-[3rem] overflow-hidden group">
                        ...
                        <CardContent className="p-8 space-y-8">
                            <div className="bg-white/5 p-8 rounded-[2rem] border border-white/5 text-center group-hover:border-blue-500/30 transition-all relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-50"></div>
                                <TrendingUp className="h-10 w-10 text-blue-500 mx-auto mb-3" />
                                <p className="text-4xl font-black text-white tracking-tighter italic">1.8<span className="text-blue-500">m</span></p>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mt-2">AVG Fulfillment Delay</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-end">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-lg font-black text-white">{allMedicines.length - totalAlerts} / {allMedicines.length}</p>
                                    </div>
                                    <Badge className="bg-blue-500/20 text-blue-400 border-none rounded-lg text-[10px] font-black uppercase">Optimal</Badge>
                                </div>
                            ...
                            <div className="pt-8 border-t border-white/5 space-y-3">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Neural Gateways</p>
                                <Button
                                    variant="outline"
                                    className="w-full justify-between h-14 bg-white/5 border-white/10 text-slate-300 hover:bg-white/10 hover:text-white rounded-2xl font-black transition-all group/btn"
                                    onClick={() => navigate('/pharmacist/inventory')}
                                >
                                    <span className="flex items-center"><Package className="mr-3 h-4 w-4 text-blue-500" /> REPOSITORY MGT</span>
                                ...
                                >
                                    <span className="flex items-center"><ShoppingCart className="mr-3 h-4 w-4 text-purple-500" /> DIRECT ACQUISITION</span>
                                    <ArrowRight className="h-4 w-4 group-hover/btn:translate-x-1 transition-transform" />
                                </Button>
                                <Button
                                    variant="outline"
                    ...
                    {/* ALERT TERMINAL */}
                    <div className="p-8 bg-gradient-to-br from-rose-950/40 to-slate-900 border border-rose-500/20 shadow-2xl rounded-[3rem] space-y-6 relative overflow-hidden group">
                        ...
                        <div className="flex items-center justify-between relative z-10">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-rose-500/20 rounded-lg border border-rose-500/30 animate-pulse">
                                    <AlertTriangle className="h-5 w-5 text-rose-500" />
                                </div>
                                <h4 className="font-black text-xs uppercase tracking-[0.3em] text-rose-400">Critical Breach</h4>
                            </div>
                        ...
                        <div className="space-y-4 relative z-10">
                            {alerts.lowStock?.length > 0 ? alerts.lowStock.slice(0, 2).map((item: any) => (
                                <div key={item._id} className="p-4 bg-white/5 rounded-2xl border border-white/5 flex justify-between items-center group-hover:bg-white/10 transition-all">
                                    <div className="space-y-1">
                                        <span className="font-black text-slate-200 text-xs uppercase tracking-tight">{item.name}</span>
                                        <p className="text-[10px] font-bold text-slate-500 italic">Depletion imminent</p>
                                    </div>
                                    <span className="text-rose-500 font-black text-sm tracking-tighter">{item.currentStock} REMAINING</span>
                                </div>
                            )) : (
                                <div className="p-6 bg-white/5 rounded-2xl border border-white/5 text-center italic text-slate-500 text-xs">
                        ...
                        <Button
                            variant="link"
                            className="p-0 text-rose-400 font-black text-[10px] uppercase h-auto group-hover:text-rose-300 transition-colors relative z-10 pl-2"
